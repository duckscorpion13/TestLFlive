...
@property (atomic, assign) BOOL isEncode;
...
- (void)start: (BOOL)isEnc {
    dispatch_async(self.rtmpSendQueue, ^{
        [self _start: isEnc];
    });
}

- (void)_start: (BOOL)isEnc {
    if (!_stream) return;
        ...
        if (!_self.isSending && _self.buffer.list.count > 0) {
            ...
            if ([frame isKindOfClass:[LFVideoFrame class]]) {
                if (!_self.sendVideoHead) {
                    ...
                    if(_isEncode) {
                        [_self sendEncVideo:(LFVideoFrame *)frame];
                    } else {
                        [_self sendVideo:(LFVideoFrame *)frame];
//                        [_self sendEncVideoVerify:(LFVideoFrame *)frame];
                    }
                }
            } else {
...
- (void)sendEncVideoVerify:(LFVideoFrame *)frame {
    NSInteger i = 0;
    NSInteger j = 0;
    ...
    unsigned char *encFrame = (unsigned char *)malloc(rtmpLength);
...
- (void)sendEncVideo:(LFVideoFrame *)frame {
    NSInteger i = 0;
    NSInteger j = 0;
    NSInteger rtmpLength = frame.data.length + 9;
    unsigned char *body = (unsigned char *)malloc(rtmpLength);
