...
- (void)startLive:(LFLiveStreamInfo *)streamInfo isEnc:(BOOL) isEnc{
    if (!streamInfo) return;
    _streamInfo = streamInfo;
    _streamInfo.videoConfiguration = _videoConfiguration;
    _streamInfo.audioConfiguration = _audioConfiguration;
    [self.socket start: isEnc];
}
...
- (void)captureOutput:(nullable LFVideoCapture *)capture pixelBuffer:(nullable CVPixelBufferRef)pixelBuffer isBeauty:(BOOL)isBeauty{
    
    if (isBeauty && self.delegate && [self.delegate respondsToSelector:@selector(handleVideo:frame:)]) {
        [self.delegate handleVideo:self frame:pixelBuffer];
    }
    
    if (self.uploading && !isBeauty) [self.videoEncoder encodeVideoData:pixelBuffer timeStamp:NOW];
}
